...
(defprotocol ISQLDriver
  ...
  (included-schemas ^java.util.Set [this]
    "*OPTIONAL*. Set of string names of schemas to syncing tables from.")
...
(defn fast-active-tables
  ...
  (with-resultset-open [rs-seq (.getSchemas metadata)]
    (let [all-schemas (set (map :table_schem rs-seq))
          schemas     (if (nil? (included-schemas driver))
                        (set/difference all-schemas (excluded-schemas driver))
                        (set/intersection all-schemas (included-schemas driver))) ]
...
(defn ISQLDriverDefaultsMixin
  ...
  {:active-tables        fast-active-tables
   ...
   :included-schemas     (constantly nil)
